...
Moving variables to a separate config file (like `config.mk`) is a best practice. It makes your Makefile more flexible: you can change how programs run or where data comes from without touching the core logic. This is especially important in research where you might need to run the same workflow with different tools or data sources.
