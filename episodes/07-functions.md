...
After dropping a new file (for example, `dorian_gray.txt`) into `books/`, the `wildcard` and `patsubst` functions automatically pick it up — you should not need to edit any rules. A typical run looks like:
...
```output
python countwords.py books/abyss.txt abyss.dat
python countwords.py books/dorian_gray.txt dorian_gray.dat
python countwords.py books/isles.txt isles.dat
python countwords.py books/last.txt last.dat
python countwords.py books/sierra.txt sierra.dat
python testzipf.py  abyss.dat  dorian_gray.dat  isles.dat  last.dat  sierra.dat > results.txt
```

```output
Book          First   Second  Ratio
abyss          4044    2807    1.44
dorian_gray    5031    2746    1.83
isles          3822    2460    1.55
last          12244    5566    2.20
sierra         4242    2469    1.72
```

The exact numbers depend on which version of the text you downloaded and whether you trimmed the Project Gutenberg header/footer, but the new ratio should land in roughly the same neighbourhood as the others (~1.4–2.2), supporting Zipf's law. The key takeaway is that *no Makefile changes were required*: the wildcard/patsubst functions automatically expanded `TXT_FILES` and `DAT_FILES` to include the new book.
